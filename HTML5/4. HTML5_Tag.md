...
* 여러개의 리스트에서 복수개의 아이템을 선택할 때 사용합니다.<br/>
  서버에 전송되는 데이터는 select 요소이 name 어트리뷰트를 키로, option 요소의 value 어트리뷰트를 값으로 하여 key=value 의 형태로 전송된다.
...
* `<input type="button">`과 유사하지만 input 태그는 빈 태그인 반면에 button 태그는 빈 태그가 아니라 텍스트와 이미지같은 콘텐츠를 사용할 수 있습니다.<br/>
  ...
  <button type="button" onclick="javascript:alert('호기심이 많으시군요!');">이 버튼을 누르신 당신은?</button>
