...
괄호 안에 a, b 를 매개변수(parameter)라고 하며, return 뒤에 오는 값을 반환값(return value)라고 합니다.<br/>
...
함수명 뒤에 인자(argument)로 계산하고자 하는 값을 넣어 호출을 하여 함수를 실행하고 실행결과에 해당하는 반환값을 받습니다.<br/>
이 것이 함수의 호출입니다.<br/>
...
let result = multiply(23, 34); // (2) - 함수의 호출

console.log(result); // (4) - 나머지 구문 실행
```
...
매개변수는 변수의 일종으로 함수의 호출시마다 인자가 매개변수의 값으로 대입됩니다.<br/>
위의 코드에서 multiply(23, 34)와 같이 호출을 하면 매개변수에는 a = 23, b = 34 의 값이 대입되고 나머지 코드가 실행됩니다.<br/>
함수를 호출할 때에도 반드시 인자의 값이 매개변수로 들어가는 것이기에, 함수의 인자값을 적어줘야합니다.<br/>
인자에 변수를 써주고, 매개변수에 새로운 값을 대입해도 인자로 써준 변수의 값은 변경되지 않습니다.<br/>
...
function multiply(x) {
  return two * x; // ReferenceError: two is not defined
}
```
...
값으로 사용할 수 있는 JS 함수는 1 급 함수입니다.<br/>

1.  함수를 변수나 데이터에 할당할수 있으며,
2.  함수를 인자로 전달할 수 있고,
3.  함수를 리턴 할 수 있기 떄문입니다.

이러한 1 급 함수의 특징을 가지고 여러 가지 프로그래밍 기법으로 활용할 수 있습니다.<br/>
