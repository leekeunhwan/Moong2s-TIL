...
// Each nucleotide has an impact factor, which is an integer.Nucleotides of types A, C, G
// and T have impact factors of 1, 2, 3 and 4, respectively.
...
function solution(S, P, Q) {
  ...
  for (let i = 0; i < P.length; i++) {
    const dnaString = S.slice(P[i], Q[i] + 1);

    if (dnaString.indexOf("A") !== -1) {
      answer.push(matchedObj["A"]);
      continue;
    }

    if (dnaString.indexOf("C") !== -1) {
      answer.push(matchedObj["C"]);
      continue;
    }

    if (dnaString.indexOf("G") !== -1) {
      answer.push(matchedObj["G"]);
      continue;
    }

    answer.push(matchedObj["T"]);
  }
